...
class MuseTalkModel:
    ...
    def _create_video_with_audio(self, frame_paths, audio_path, output_path, fps):
        try:
            ...
            # Create video with audio in a single command
            # This is more reliable than creating video first, then adding audio
            cmd = f"ffmpeg -y -framerate {fps} -i '{frame_pattern}' -i '{audio_path}' \
                  -c:v libx264 -preset medium -profile:v high -crf 23 -pix_fmt yuv420p \
            ...
            if result != 0:
                ...
                # Try an alternative approach with simpler settings
                ...
                if result != 0:
                    raise Exception(f"Both ffmpeg attempts failed, exit codes: {result}")
            ...
            try:
                ...
                # Try to create a video with just the frames, no audio
                emergency_cmd = f"ffmpeg -y -framerate {fps} -i '{frame_pattern}' -c:v libx264 '{output_path}'"
